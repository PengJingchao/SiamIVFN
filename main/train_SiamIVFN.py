...
config = '../experiments/siamfcpp/train/rgbt/SiamIVFN_train.yaml'

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test')
    parser.add_argument('-cfg', '--config', default=config, type=str, help='path to experiment configuration')
    parser.add_argument('-r', '--resume', default="", help=r"completed epoch's number, latest or one model path")
    parsed_args = parser.parse_args()

    # experiment config
    exp_cfg_path = osp.realpath(parsed_args.config)  # 获取当前执行脚本的绝对路径
    ...
    with Timer(name="Dataloader building", verbose=True):
        ...
        valdataloader = dataloader_builder.build(task, task_cfg.valdata)
    dataloader = [dataloader, valdataloader]
