...
class ExperimentRGBT(object):
    r"""Experiment pipeline and evaluation toolkit for RGBT dataset.

    Args:
        root_dir (string): Root directory of RGBT dataset.
        version (integer or string): Specify the benchmark version, specify as one of
            ``2013``, ``2015``, ``tb50`` and ``tb100``. Default is ``2015``.
        result_dir (string, optional): Directory for storing tracking
            results. Default is ``./results``.
